                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            @if ($code !== $languages->first()->code)
                                <button onclick="translateAll()" class="btn btn-primary" id="translate-btn">{{ __('Translate') }}</button>
                            @endif
                            @endadminCan
                        </div>
                        <div class="card-body">
                            <div class="lang_list_top">
                                <ul class="lang_list">
                                    @foreach ($languages = allLanguages() as $language)
                                        <li><a id="{{ request('code') == $language->code ? 'selected-language' : '' }}" href="{{ route('admin.blogs.edit', ['blog' => $blog->id, 'code' => $language->code]) }}"><i class="fas {{ request('code') == $language->code ? 'fa-eye' : 'fa-edit' }}"></i>
                                                {{ $language->name }}</a></li>
                                    @endforeach
                                </ul>
                            </div>
                            ...
                            <div class="mt-2 alert alert-danger" role="alert">
                                ...
                                @endphp
                                <p>{{ __('admin.Your editing mode') }}:<b> {{ $current_language?->name }}</b></p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header d-flex justify-content-between">
                                <h4>{{ __('Edit Post') }}</h4>
                                <div>
                                    <a href="{{ route('admin.blogs.index') }}" class="btn btn-primary"><i class="fa fa-arrow-left"></i>{{ __('Back') }}</a>
                                </div>
                            </div>
                            <div class="card-body">
                                <form action="{{ route('admin.blogs.update', [
                                        'blog' => $blog->id,
                                        'code' => $code,
                                    ...
                                    <div class="row">
                                        @if ($code == $languages->first()->code)
                                            <div class="form-group col-md-8 offset-md-2">
                                                ...
                                                <div id="image-preview" class="image-preview" @if ($blog->image ?? false) style="background-image: url({{
                                                    asset($blog->image) }}); background-size: cover; background-position: center center;" @endif>
                                        ...
                                        <div class="form-group col-md-8 offset-md-2">
                                            ...
                                            <input data-translate="true" type="text" id="title" class="form-control" name="title" value="{{ $blog->getTranslation($code)->title }}">
                                        </div>
                                        @if ($code == $languages->first()->code)
                                            <div class="form-group col-md-8 offset-md-2">
                                        ...
                                        <div class="form-group col-md-8 offset-md-2">
                                            ...
                                            <textarea name="description" id="" cols="30" rows="10" class="summernote">{!! $blog->getTranslation($code)->description !!}</textarea>
                                        </div>
                                        @if ($code == $languages->first()->code)
                                            <div class="form-group col-md-8 offset-md-2">
                                                <label>
                                        ...
                                        <div class="form-group col-md-8 offset-md-2">
                                            ...
                                            <input data-translate="true" type="text" class="form-control" name="seo_title" value="{{ $blog->getTranslation($code)->seo_title }}">
                                        </div>

                                        <div class="form-group col-md-8 offset-md-2">
                                            <label>{{ __('admin.SEO Description') }}</label>
                                            <textarea data-translate="true" name="seo_description" id="" cols="30" rows="10" class="form-control text-area-5">{{ $blog->getTranslation($code)->seo_description }}</textarea>
                                        </div>
