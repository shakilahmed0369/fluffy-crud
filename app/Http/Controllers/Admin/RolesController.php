...
class RolesController extends Controller
{
    ...
    {
        abort_unless(checkAdminHasPermission('role.view'), 403);
    ...
    {
        abort_unless(checkAdminHasPermission('role.create'), 403);
    ...
    {
        abort_unless(checkAdminHasPermission('role.store'), 403);
    ...
    {
        abort_unless(checkAdminHasPermission('role.edit'), 403);
        $permissions = Permission::all();
        $permission_groups = Admin::getPermissionGroup();
    ...
    {
        abort_unless(checkAdminHasPermission('role.update'), 403);
        if (!empty($request->permissions)) {
    ...
    {
        abort_unless(checkAdminHasPermission('role.delete'), 403);
    ...
    {
        abort_unless(checkAdminHasPermission('role.assign'), 403);
    ...
    public function assignRoleUpdate(Request $request)
    {
        ...
        $messages = [
            'user_id.required' => trans('You must select an admin'),
            'user_id.exists' => trans('Admin not found'),
            'role.required' => trans('You must select role'),
            'role.array' => trans('You must select role'),
            'role.*.required' => trans('You must select role'),
            'role.*.string' => trans('You must select role'),
        ];
